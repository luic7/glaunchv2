...
- Hardcoded default window management keys (can be made configurable later):
  - F3: win_delete (close window)
  - F4: win_prev (previous window)
  - F12: win_other (cycle non-configured windows)
  - win_center_mouse enabled
...
- Connects to `changed::shortcuts` GSettings signal in `enable()`
- `_reloadShortcuts()` method unbinds old keybindings and recreates Config/Launcher
- Signal handler properly disconnected in `disable()`
...
1. **Make window management keys configurable** - Currently F3, F4, F12 are hardcoded for win_delete, win_prev, win_other
2. **Add modifier key support** - Allow Ctrl+F1, Alt+F2, etc.
3. **Edit existing shortcuts** - Currently can only add/delete, not edit
4. **Import/export settings** - Backup and restore configurations
